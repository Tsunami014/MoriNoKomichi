...
void updatePoss(std::vector<TaskWidget*> sections[4], QGraphicsItemGroup* groups[4]) {
    int sectPadding = 50;
    qreal sectWid = 0;
    unsigned int sectHei = 0;
    ...
    for (int i = 0;i < 4;i++) {
        ...
        if (i != 3) {
            ...
            if (i == 0 || i == 2) {
                ...
                unsigned int mxHei = 0;
                for (int j = 0; j < heights.size(); j++) {
                    mxHei = qMax(mxHei, heights[j]);
                }
                sectHei = qMax(sectHei, mxHei + sectPadding);
            }
        }
